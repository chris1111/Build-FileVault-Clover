...
# build FileVault2 drivers-x64 packages
if [[ -d "${SRCROOT}/CloverV2/EFI/CLOVER/drivers64" && ${NOEXTRAS} != *"CloverEFI"* ]]; then
echo "================= FileVault2 Drivers =================="
addGroupChoices --title="FileVault2" --description="FileVault2"  \
--enabled="!choices['UEFI.only'].selected"     \
--visible="!choices['UEFI.only'].selected"     \
"FileVault2"

packagesidentity="${clover_package_identity}".FileVault2
local drivers=($( find "${SRCROOT}/CloverV2/drivers-Off/FileVault2" -type f -name '*.efi' -depth 1 | sort -f ))
local driverDestDir='/EFIROOTDIR/EFI/CLOVER/drivers64'
for (( i = 0 ; i < ${#drivers[@]} ; i++ ))
do
local driver="${drivers[$i]##*/}"
...
# End FileVault2 drivers-x64 packages

# build FileVault2UEFI packages
if [[ -d "${SRCROOT}/CloverV2/drivers-Off/FileVault2UEFI" ]]; then
echo "=================== drivers FileVault2 UEFI ====================="
addGroupChoices --title="FileVault2UEFI" --description="Drivers UEFI for FileVault2" "FileVault2UEFI"
packagesidentity="${clover_package_identity}".FileVault2UEFI
local drivers=($( find "${SRCROOT}/CloverV2/drivers-Off/FileVault2UEFI" -type f -name '*.efi' -depth 1 ))
...
--subst="INSTALLER_CHOICE=$packageRefId" MarkChoice
buildpackage "$packageRefId" "${driverName}" "${PKG_BUILD_DIR}/${driverName}" "${driverDestDir}"
addChoice --group="FileVault2UEFI"  --title="$driverName"                \
--start-selected="choicePreviouslySelected('$packageRefId')"  \
--pkg-refs="$packageRefId"  "${driverName}"
rm -R -f "${PKG_BUILD_DIR}/${driverName}"
done
fi
# End build FileVault2UEFI packages
