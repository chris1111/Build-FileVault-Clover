...
#  Micky1979 2016
#  Modified by chris1111 21/09/2018
...
MainPostBuildScript() {
    ...
    if [[ "${TARGETARCH}" = IA32 ]]; then
      ...
      mkdir -p "$CLOVER_PKG_DIR"/drivers-Off/FileVault2UEFI
      mkdir -p "$CLOVER_PKG_DIR"/drivers-Off/FileVault2
    ...
    if [[ "$TARGETARCH" = X64 ]]; then
     ...
     if (( $NOBOOTFILES == 0 )); then
      ...
      rm -Rf "$CLOVER_PKG_DIR"/drivers-Off/FileVault2* 2> /dev/null
      rm -Rf "$CLOVER_PKG_DIR"/drivers-Off/FileVault2UEFI* 2> /dev/null
      ...
      # FileVault drivers
      echo "Copy FileVault drivers:"
      binArray=( AppleImageCodec AppleUITheme AppleKeyFeeder AppleKeyAggregator FirmwareVolume AptioInputFix HashServiceFix)
      for efi in "${binArray[@]}"
      do
        copyBin "$BUILD_DIR_ARCH"/$efi.efi "$CLOVER_PKG_DIR"/drivers-Off/FileVault2UEFI/$efi-64.efi
        copyBin "$BUILD_DIR_ARCH"/$efi.efi "$CLOVER_PKG_DIR"/drivers-Off/FileVault2/$efi-x64.efi
      done
...
#done
      ...
      case "$EXT_DOWNLOAD" in
        ...
        0 | 2 | 3)
          ...
          copyBin "$APTIO_BUILD_DIR_ARCH"/AptioInputFix.efi "$CLOVER_PKG_DIR"/drivers-Off/FileVault2UEFI/AptioInputFix-64.efi
 ...
 #         copyBin "$APFS_BUILD_DIR_ARCH"/ApfsDriverLoader.efi "$CLOVER_PKG_DIR"/drivers-Off/drivers64/ApfsDriverLoader-64.efi 
        ...
        binArray=( AppleUISupport AptioInputFix )
            for efi in "${binArray[@]}"
            do
                copyBin "$APFS_BUILD_DIR_ARCH"/$efi.efi "$CLOVER_PKG_DIR"/drivers-Off/FileVault2UEFI/$efi-64.efi
                copyBin "$APFS_BUILD_DIR_ARCH"/$efi.efi "$CLOVER_PKG_DIR"/drivers-Off/FileVault2/$efi-x64.efi
            done
          ;;
      esac
      # drivers64UEFI
